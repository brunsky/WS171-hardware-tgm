...
static const char *readline()
{
     ...
     * mean "buffer consumed completely". If it points to a character, then
    ...
    if (*s_ATBufferCur == '\0') {
        ...
        *s_ATBufferCur = '\0';
        ...
        if (p_eol == NULL) {
            ...
            //see how long is the string line
            len = strlen(s_ATBufferCur);

            //move partial line to the front of the buffer.....
            memmove(s_ATBuffer, s_ATBufferCur, len + 1);

            //setting next read postion
            p_read = s_ATBuffer + len;

            //reset read cursor to the front of the buffer;
            s_ATBufferCur = s_ATBuffer;
        }
    ...
    while (p_eol == NULL) {
        ...
        //we shall halt here until there is a data to read
        ...
        if (count > 0) {
            ...
            if(count == 0) {
            	...
            	//We are switching to noblocking mode, nothing to read
...
void at_close()
{
    ...
    if (s_fd >= 0) {
    	...
    	//SET File Read Description to NON-Block; making read loop escapes

        int err = close(s_fd);

        if(err < -1){

        	LOGE("Error when closing at channel");
                                 ...
                                 ATResponse **pp_outResponse)
{
    ...
    err = at_send_command_full (command, SINGLELINE, responsePrefix,
                                    pdu, TGM_TIMEOUT, pp_outResponse);
