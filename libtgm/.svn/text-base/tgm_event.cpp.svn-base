/* //device/libs/telephony/tgm_event.cpp
 **
...
static struct tgm_event * watch_table[MAX_FD_EVENTS];
static struct tgm_event timer_list;
static struct tgm_event pending_list; //a list that either timer is out or persistent command

#define DEBUG 0

#if DEBUG
#define dlog(x...) LOGD( x )
static void dump_event(struct tgm_event * ev)
{
...
static void init_list(struct tgm_event * list)
{
    memset(list, 0, sizeof(struct tgm_event));
    list->next = list;
    list->prev = list;
    list->fd = -1;
}
...
//Linked list
static void addToList(struct tgm_event * ev, struct tgm_event * list)
{
    ev->next = list;
    ev->prev = list->prev;
    ev->prev->next = ev;
...
static void removeFromList(struct tgm_event * ev)
{
    dlog("~~~~ Removing event ~~~~");
    dump_event(ev);
...
static void removeWatch(struct tgm_event * ev, int index)
{
    ...
    if (ev->fd+1 == nfds) {
        ...
        for (int i = 0; i < MAX_FD_EVENTS; i++) {
            struct tgm_event * rev = watch_table[i];
...
static void processTimeouts()
{
    ...
    MUTEX_ACQUIRE();
    struct timeval now;
    struct tgm_event * tev = timer_list.next;
...
static void firePending()
{
    ...
    struct tgm_event * ev = pending_list.next;
    while (ev != &pending_list) {


        struct tgm_event * next = ev->next;
...
{
    dlog("~~~~ tgm_event_set %x ~~~~", (unsigned int)ev);
    memset(ev, 0, sizeof(struct tgm_event));
    ev->fd = fd;
    ev->index = -1;
    ev->persist = persist;
    ev->func = func;
...
void tgm_event_loop()
{
    ...
    for (;;) {
        ...
        if (n < 0) {
            ...
            LOGE("tgm_event: select error (%d)", errno);
            // bail?
            return;
        }
